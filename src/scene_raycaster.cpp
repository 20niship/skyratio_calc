...
// UV球メッシュを生成
void generate_uv_sphere(const Vec3& center, double radius, int segments, int rings, std::vector<float>& vertices, std::vector<unsigned int>& indices) {
...
SceneRaycaster::SceneRaycaster(SceneRaycaster&& other) noexcept : build_dirty(other.build_dirty), boxes(std::move(other.boxes)), spheres(std::move(other.spheres)), vertices(std::move(other.vertices)), indices(std::move(other.indices)), bvh(other.bvh) { other.bvh = nullptr; }
