...
class Box:
    ...
    def __init__(self, center: Vec3, size: Vec3):
        ...
        self.size = size
        # 最小・最大境界を計算
        self.min = Vec3(center.x - size.x / 2, center.y - size.y / 2, center.z - size.z / 2)
        self.max = Vec3(center.x + size.x / 2, center.y + size.y / 2, center.z + size.z / 2)
...
class SceneRaycasterPython:
    ...
    def add_box(self, pos: List[float], size: List[float], euler: List[float] | None = None):
        """
        シーンにボックスを追加
        ...
        if euler is not None and any(abs(angle) > 1e-6 for angle in euler):
            raise NotImplementedError("回転（オイラー角）はPython実装ではサポートされていません。軸平行ボックスのみサポートされています。")
    ...
    def check(self) -> List[float]:
        ...
        for checkpoint_list in self.checkpoints:
            ...
            for t in range(theta_steps + 1):
                ...
                theta_area = (math.sin(theta_end) ** 2 - math.sin(theta_start) ** 2) / 2.0
