...
def test_large_wall_blocks_half_hemisphere():
    ...
    assert 0.45 < sky_ratio < 0.55, f"大きな壁が半球を遮る場合、天空率は45-55%の範囲のはずが {sky_ratio * 100:.2f}%"
...
def test_many_small_objects_far_away():
    ...
    assert sky_ratio > 0.90, f"小さなオブジェクトが遠くにある場合、天空率は90%以上のはずが {sky_ratio * 100:.2f}%"
...
def test_no_obstacles():
    ...
    assert abs(sky_ratio - 1.0) < 0.01, f"障害物がない場合、天空率は100%のはずが {sky_ratio * 100:.2f}%"
...
def test_completely_enclosed():
    ...
    assert sky_ratio < 0.50, f"真上に大きな天井がある場合、天空率は50%以下のはずが {sky_ratio * 100:.2f}%"
